...
Now, let's see the distribution of independent variable. How to fix the graph? When the graph is fixed, add the following line of code `scale_x_discrete(labels = c("Absent", "Present"))` to the graph.

```{r}
#| message: false

ggplot(appel_loyle) +
  geom_histogram(aes(x = as.factor(pcj)), stat = "count") +
  theme_light() +
