...
-   Add `system_category` from WhoGov

-   Transform the dependent variable to the polynomial in second power


```{r}
model_in_fix = lm(I(Perceptions_of_corruption)^2 ~ n_individuals + indep + system_category, whr_whogov)
...
Try adding `system_category` from `WhoGov` dataset as a control variable, and check the results for Q-Q
